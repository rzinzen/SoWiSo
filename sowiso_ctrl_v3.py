...
pitch_pos_in_degree = 0
global DC
DC = 100
global move_time
move_time = 0
global degree_yaw_turn_time
degree_yaw_turn_time = 1
global degree_ext_time
degree_ext_time = 0.03
global degree_retract_time
degree_retract_time = 0.03
global ext_retr_conversion
ext_retr_conversion = 1
...
def run_motor(move_time):   # runs motors for specified time and recalculates yaw or pitch position 
    ...
    if (yaw_or_pitch_move == 'yaw'):
        print('old yaw poistion was ',yaw_pos)
        ...
        yaw_pos_in_deg = yaw_pos*degree_yaw_turn_time
    ...
    if (yaw_or_pitch_move == 'pitch'):
        print('old pitch poistion was ',pitch_pos)
        if (direction_move == 'extend'):
            ...
            print('...SOMETHING WENT WRONG !!!!!!!...')
        pitch_pos_in_degree = pitch_pos*degree_ext_time
        if (pitch_pos_in_degree < 0):
            pitch_pos_in_degree = 0
        elif (pitch_pos_in_degree>90):
            pitch_pos_in_degree = 90
        print('new pitch poistion is ~',pitch_pos_in_degree, '° extension')
...
def find_yaw_parameters():      # determines time needed for yaw revolution: 'full_yaw_turn_time', 'degree_yaw_turn_time', & zeros 'yaw_pos'
    ...
    full_yaw_turn_time = round(stop-start,3)    #determine time elapsed and store
    time.sleep(1)                                   #short rest interval
...
def find_pitch_parameters():    # determines 'full_ext_time', 'degree_ext_time', 'full_retract_time', 'degree_retract_time', & zeros 'pitch_pos'
    ...
    pitch_pos_in_degree = 0                             #zeros current pitch position
...
def return_to_origin():
    ...
    print ('Moving ',yaw_or_pitch,' to ',direction,' with ',DC,'% DC...')
    yaw_sensor()                                    #activtate sensor() function
    stop_motors()                                   #stops all motors
    yaw_pos = 0
    yaw_pos_in_deg = 0    
    ...
    ## Find Pitch origin
    if (pitch_pos_in_degree < 10):
        start_motor('pitch', 'retract', 100)
        time.sleep((pitch_pos_in_degree * degree_retract_time)+2)
    else:
        start_motor('pitch', 'retract', 100)
        if(pitch_pos_in_degree>80):
            time.sleep(2)
        pitch_sensor()
...
def measure_solar():            # measures value on adc and defines voltage variables 
    ...
    R1 = 470                        # Resistor 1 in Ohm
    R2 = 47+4.7                     # Resistor 2 in Ohm
...
def assess_light():             # initial light level assessment if optimization is sensible 
    ...
    global light_level
    global light_sufficient
    
    light_sufficiency_threshold = 2.5    #MUST SET!! -- DETERMINES estimated voltage level above which optimzation makes sense
    darkness_threshold = 1              #MUST SET!! -- DETERMINES estimated voltage level below which it is considered to be dark
...
def optimize_yaw():
    ...
    while (optimize == True):
        ...
        if (volt_solar_t0 <= volt_solar_t0):
...
def workflow():
    ...
    # return_to_origin()
    # assess light()            # initial light level assessment if optimization is sensible
    ...
    sowiso_stoptime = now.replace(hour=21, minute=0, second=0, microsecond=0)
